...
void NimbleController::AddCharacteristicSecurity(const struct ble_gatt_svc_def* svcs) {
  ...
  for (int si = 0; svcs[si].type != 0; si++) {
    chrs = (struct ble_gatt_chr_def*) svcs[si].characteristics;
...
void NimbleController::StartAdvertising() {
  ...
  if (systemTask.GetSettings().GetBlePairingAllowed()) {
    ...
    rc = ble_store_util_bonded_peers(&peer_addr, &num_peers, 1);
    ASSERT(rc == 0 && num_peers == 1);

    rc = ble_gap_wl_set(&peer_addr, 1);
    ASSERT(rc == 0);
  }
  ...
  switch (event->type) {
    ...
    case BLE_GAP_EVENT_PASSKEY_ACTION:
      ...
      if (!systemTask.GetSettings().GetBlePairingAllowed() ||
        ...
        do {
          if (systemTask.GetSettings().GetBleSecured()) {
            ble_ll_rand_data_get((uint8_t*) &passkey_rand, 4);
          } else {
